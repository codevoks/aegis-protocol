...
**Right now (Phase 0):** this repository is 100% documentation. A `git clone` reconstructs the
entire project as it currently exists — there is no toolchain, dependency, or local-service state to
regenerate yet, because no code has been written.

**From Phase 1 onward:** the repository will gain a Rust/Anchor workspace, a TypeScript SDK/app, and
local validator state, all of which are reproducible from source with no paid service:

```bash
make setup   # verify/install the pinned toolchain (Solana CLI, Anchor, Surfpool, Node)
make build   # anchor build
make test    # full offline test suite — no network, no secrets, no faucet
make demo    # scripted end-to-end scenario against a local validator
```

The exact install commands, pinned versions, and verification steps for `make setup` are specified in
[`docs/phases/phase-01-foundation.md`](docs/phases/phase-01-foundation.md) §3–5. Everything `.gitignore`
excludes from then on (`target/`, `node_modules/`, `.anchor/`, local validator ledgers, build caches) is
mechanically regenerated by these commands — never hand-crafted, and never required to understand or
review the project.
