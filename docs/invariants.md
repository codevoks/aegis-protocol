...
| INV-ACC-06 | `total_supply_shares == 0 ⟺ total_supply_assets == 0`, and likewise for borrow. (No orphaned assets without shares, or shares without assets.) **Phase 13 finding (F-13-01, `docs/security/findings.md`):** the supply-side direction can be violated in its literal wording after a bad-debt event leaves `fee_position` holding sub-base-unit dust shares and the market's remaining lender withdraws in full — confirmed harmless (the dust's implied value is proven `0` for a future deposit of any size, never merely small), left unfixed, and permanently regression-tested (`tests/adversarial/orphaned_fee_shares.rs`). The borrow-side direction has no such exception: F-10-02 (also cited below) was a real, uncollectible-obligation violation and was fixed. | all ix | `F-INV-06` | 4 |
